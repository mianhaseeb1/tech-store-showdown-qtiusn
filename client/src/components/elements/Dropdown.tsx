...
export function DropdownItem({
    className,
    ...props
}: { className?: string } & (Omit<React.ComponentPropsWithoutRef<'button'>, 'as' | 'className'> | Omit<React.ComponentPropsWithoutRef<typeof Link>, 'className'>)) {
    let classes = clsx(
        className,
        // Base styles
    ...
    return <Headless.MenuItem>{'href' in props ? <Link {...props} className={classes} /> : <button type="button" {...props} className={classes} />}</Headless.MenuItem>
}
...
export function DropdownHeading({ className, ...props }: { className?: string } & Omit<Headless.MenuHeadingProps, 'as' | 'className'>) {
    return (
        <Headless.MenuHeading
            {...props}
            className={clsx(className, 'col-span-full grid grid-cols-[1fr,auto] gap-x-12 px-3.5 pb-1 pt-2 text-sm/5 font-medium text-zinc-500 sm:px-3 sm:text-xs/5 dark:text-zinc-400')}
        />
    )
}
